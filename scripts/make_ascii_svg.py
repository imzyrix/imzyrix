...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "imzyrix-ascii.svg")
...
FRAME = "#4b4f7a"
TITLE_TEXT = "#8b93a7"
INK = "#d5d9e6"      # the single ascii color (matches Andrew6rant)
CURSOR = "#d5d9e6"
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">imzyrix@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'imzyrix@github:~$ whoami <tspan fill="{INK}">imzyrix</tspan></text>')
