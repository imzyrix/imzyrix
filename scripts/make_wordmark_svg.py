"""
Render "ZX" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
FONT_PATH = os.environ.get(
    "WORDMARK_FONT",
    # macOS ships Futura (the render in the original repo uses it). On
    # Linux fall back to a bold sans with an even stroke weight. Override
    # with WORDMARK_FONT to point at any TTF/TTC you prefer.
...
TEXT = os.environ.get("WORDMARK_TEXT", "ZX")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">imzyrix@github: ~$ ./wordmark.sh --3d</text>')
